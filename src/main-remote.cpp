...
void loop()
{
	...
	delay(3);
	int leverValue = analogRead(leverPot) - 266; // Su 676  Giu 274
	delay(3);
	current_time = millis();
	...
	sentData.speedmotorLeft = map(strValue, 0, analogReadMax, -255, 255);
	sentData.speedmotorRight = map(accValue, 0, analogReadMax, -255, 255);
	// -------------------------------------------- //
	esp_err_t result = -1;
