...
uint16_t MotorControl::_free_pwm_unit = 0;	// che schifo di inizializzazione WARNING,
uint16_t MotorControl::_free_pwm_timer = 0; // usate come variabili globali anche se rifedinizione nella classe
uint16_t MotorControl::_pwm_freq = 4000;	// xxx
...
void MotorControl::setSpeed(int speed)
{
	...
	int speed_sign = (speed > 0) - (speed < 0); // sta variabile a che serve?
	if (_start_pwm != 0)
	{
		speed_abs = map(speed_abs, 0, _max_resolution, _start_pwm, _max_resolution);
	}
	float duty = (float)speed_abs;			 // cast a float
	duty = duty / (_max_resolution / 100.0); // se la max resolution e' fissa perche' ricalcolarla ogni volta
	if (speed < 0)
	{
		mcpwm_set_duty(_pwm_unit, _pwm_timer, MCPWM_GEN_A, 0);
		mcpwm_set_duty(_pwm_unit, _pwm_timer, MCPWM_GEN_B, duty); // in base a come viene eccitato il motore inverte il min ed il max.
	}
	else if (speed > 0)
	{
	...
	{
		mcpwm_set_duty(_pwm_unit, _pwm_timer, MCPWM_GEN_A, 100); // mi pare di capire che nel caso di speed = 0 i due pin vengono settati entrambi a 100 (100 - 100 = 0, quindi nessuna eccitazione??)
